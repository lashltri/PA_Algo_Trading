...
```{r ngmacd,  fig.cap="NG=F: MACD line (blue), signal line (red), histogram (dark grey), and trading signals (grey).", out.width="70%", fig.keep="last", message=FALSE, warning=FALSE}
...
df <- as.data.frame(t(df))   # re-row it
...
```{r agBTC, echo=FALSE, fig.cap="GC=F: ARMA-GARCH vs Buy and Hold", out.width="70%", fig.keep="last", message=FALSE, warning=FALSE, fig.height=5}
...
cum_ag <- exp(cumsum(na.omit(BACKT$AG$`BTC-USD`))) - 1
cum_bh <- exp(cumsum(na.omit(BACKT$BH$`BTC-USD`[index(BACKT$AG$`BTC-USD`)]))) - 1
