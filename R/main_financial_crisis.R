...
BACKT$GARCH <- list(SSMI = strat_ag_arma_garch(price = px_list$SSMI$SSMI.Adjusted,
                                            model = "garch(1,1)",
                                            start_out_sample = "2007-01-01"),
                 GSPC = strat_ag_arma_garch(price = px_list$GSPC$GSPC.Adjusted,
                                            model = "garch(1,1)",
...
EVAL_BT <- list(
  ...
  GARCH = lapply(BACKT$GARCH, eval_backtest)
)
...
EVAL_PORTF <- list(
  ...
  GARCH = lapply(PORTF$GARCH, eval_backtest)
)
...
#sharpe for garch / arma garch comparison
mat_sharpe_garch_crisis_comp <- mat_sharpe_crisis[, c("BH", "AG", "GARCH")]


#sharpe for main evaluation
mat_sharpe_crisis <- mat_sharpe_crisis[, setdiff(colnames(mat_sharpe), "GARCH")]
mat_drawdown_crisis <- mat_drawdown_crisis[, setdiff(colnames(mat_sharpe), "GARCH")]
...
STD_RET <- standardized_returns(
  returns = BACKT[names(BACKT) != "GARCH"],
  portf   = PORTF[names(PORTF) != "GARCH"]
)
