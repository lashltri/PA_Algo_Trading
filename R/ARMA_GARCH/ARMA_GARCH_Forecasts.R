...
strat_ag_arma_garch <- function(price, 
  ...
  #Scaling out of sample not in but negligible difference
  position_size<-position_size/mean(position_size) 
  ...
  if (return_strat == F) {
    arma_out <- arma_garch_log_ret[paste(start_out_sample,"/",sep="")]
    attr(arma_out, "signal")  <- signal[paste(start_out_sample,"/",sep="")]
    return(arma_out)
